...
async function main(projectId, location, agentId, audioFileName, encoding,
  ...
  async function detectIntentAudio() {
    ...
    const sessionPath = client.projectLocationAgentSessionPath(projectId,
        location, agentId, sessionId);
        ...
        .on('data', data => {
          if (data.recognitionResult) {
            ...
            if (result.match.intent) {
              console.log(
                  `Matched Intent: ${result.match.intent.displayName}`);
            }
            console.log(
                `Current Page: ${result.currentPage.displayName}`);
          }
        });
